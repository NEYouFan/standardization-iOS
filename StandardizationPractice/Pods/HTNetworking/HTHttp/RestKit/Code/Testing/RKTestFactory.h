...
/**
 ...
 The factory also provides for the definition of named factories for instantiating objects quickly. At initialization, there are factories defined for creating instances of `RKRequestProvider`, `RKObjectManager`, and `RKManagedObjectStore`. These factories may be redefined within your application should you choose to utilize a subclass or wish to centralize configuration of objects across the test suite. You may also define additional factories for building instances of objects specific to your application using the same infrastructure.
...
/**
 Returns the base URL with which to initialize `RKRequestProvider` and `RKObjectManager` instances created via the factory.
...
/**
 Fetches the shared `RKRequestProvider` object using the factory defined for the name `RKTestFactoryDefaultNamesClient`.
