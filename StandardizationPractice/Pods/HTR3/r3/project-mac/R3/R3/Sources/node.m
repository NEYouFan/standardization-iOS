...
#endif
#else
    if (tree->regularExpression){
        [tree->regularExpression release];
        tree->regularExpression = nil;
    }
#endif
...
int r3_tree_compile_patterns(node * n, char **errstr) {
    ...
    n->regularExpression = [NSRegularExpression regularExpressionWithPattern:@(n->combined_pattern)
                                                                     options:options
                                                                       error:&error];
    [n->regularExpression retain];
    
    if (error){
        NSLog(@"NSRegularExpression compile failed:%@", error);
        return -1;
    }
#endif
    return 0;
...
node * r3_tree_matchl(const node * n, const char * path, int path_len, match_entry * entry, NSMutableDictionary *matchedParams) {
...
#if !USE_PCRE
                NSString *keyString = r3_edge_slug_string(e);
                
                char value[pp-path+1];
                memset(value, 0, pp-path+1);
                strncpy(value, path, pp-path);
                
                NSString *valueString = [NSString stringWithUTF8String:value];
                [matchedParams setObject:valueString forKey:keyString];
#endif
                if (restlen == 0) {
                    return e->child && e->child->endpoint > 0 ? e->child : NULL;
                }
                return r3_tree_matchl(e->child, pp, (int)(pp_end - pp), entry, matchedParams);
            }
        }
    }
    ...
    if (n->pcre_pattern) {
        ...
        if (n->regularExpression){
            ...
            NSArray *array = [n->regularExpression matchesInString:@(path) options:0 range:NSMakeRange(0, path_len)];
            if (array.count > 0){
                NSTextCheckingResult *result = (NSTextCheckingResult*)array.firstObject;
                NSUInteger rangeCount = [result numberOfRanges];
                NSRange range = [result rangeAtIndex:0];
                ov[0] = (int)range.location;
                ov[1] = (int)(range.location + range.length);
                ...
                for (int i=1; i<rangeCount; i++) {
                    ...
                    if (range.length == 0){
                        ...
                        ov[i*2] = (int)range.location;
                        ov[i*2+1] = (int)(range.location+range.length);
                    }
                }
...
#if !USE_PCRE
                if (e->child && e->child->endpoint > 0 && e->has_slug){
                    NSString *key = r3_edge_slug_string(e);
                    
                    char value[substring_length+1];
                    memset(value, 0, substring_length+1);
                    strncpy(value, substring_start, substring_length);
                    
                    NSString *valueString = [NSString stringWithUTF8String:value];
                    [matchedParams setObject:valueString forKey:key];
                }
#endif
        ...
        for (i = 1; i < rc; i++)
        {
            ...
            return r3_tree_matchl( e->child, path + (ov[1] - ov[0]), restlen, entry, matchedParams);
        }
        // does not match
        return NULL;
    }
    ...
    if ( (e = r3_node_find_edge_str(n, path, path_len)) != NULL ) {
        ...
        return r3_tree_matchl(e->child, path + e->pattern_len, restlen, entry, matchedParams);
    }
    return NULL;
}
...
node * r3_tree_insert_pathl_ex(node *tree, const char *path, int path_len, route * route, void * data, char **errstr)
{
    ...
    if ( prefix_len == 0 ) {
        ...
        if ( slug_cnt > 1 ) {
            ...
            r3_node_connect(n, zstrndup(path, (p - path)), child);
            ...
            if (slug_cnt == 1) {
                ...
                if (slug_pattern_len) {
                    ...
                    opcode = r3_pattern_to_opcode(cpattern, (int)strlen(cpattern));
                    zfree(cpattern);
                } else {
                ...
                if (slug_p > path) {
                    c1 = r3_tree_create(3);
                    CHECK_PTR(c1);
                    r3_node_connectl(n, path, (int)(slug_p - path), 1, c1); // duplicate
                } else {
                    c1 = n;
                }
                ...
                int restlen = path_len - (int)((slug_p - path) + slug_len);
