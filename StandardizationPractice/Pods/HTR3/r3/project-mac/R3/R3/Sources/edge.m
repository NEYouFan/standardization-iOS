...
NSString *r3_edge_slug_string(edge *e){
    if (e->has_slug){
        char key[strlen(e->pattern)];
        memset(key, 0, strlen(e->pattern));
        
        const char *slugStart = e->pattern;
        while (*slugStart) {
            if (*slugStart == '{')
                break;
            slugStart++;
        ...
        int slugNameLen = 0;
        const char *slugName = r3_slug_find_name(slugStart, &slugNameLen);
        
        strncpy(key, slugName, slugNameLen);
